...
tile(Monitor *m)
{
	...
	for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			r = MIN(n, m->nmaster) - i;
			h = (m->wh - my - gappx * (r - 1)) / r; //h = (m->wh - my) / (MIN(n, m->nmaster) - i);
			resize(c, m->wx, m->wy + my, mw - (2*c->bw), h - (2*c->bw), 0);
			my += HEIGHT(c) + gappx; //my += HEIGHT(c);
		} else {
			r = n - i;
			h = (m->wh - ty - gappx * (r - 1)) / r;
			resize(c, m->wx + mw + g, m->wy + ty, m->ww - mw - g - (2*c->bw), h - (2*c->bw), False);
			ty += HEIGHT(c) + gappx; //h = (m->wh - ty) / (n - i);//resize(c, m->wx + mw, m->wy + ty, m->ww - mw - (2*c->bw), h - (2*c->bw), 0); //ty += HEIGHT(c);
		}
}
